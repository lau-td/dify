...
from controllers.codelight.wraps import get_app_model_with_tenant_id
from controllers.codelight import api
from controllers.codelight.internal_api.app import inner_api_only
...
class CodelightModelConfigResource(Resource):
    @setup_required
    @inner_api_only
    @get_app_model_with_tenant_id(mode=[AppMode.AGENT_CHAT, AppMode.CHAT, AppMode.COMPLETION])
...
api.add_resource(
    CodelightModelConfigResource,
    "/tenants/<uuid:tenant_id>/apps/<uuid:app_id>/model-config",
)
